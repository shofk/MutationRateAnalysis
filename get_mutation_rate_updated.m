% Mutation Rate Data Processing & Mutation Rate Calculating
% @Usage: Run, then the program will ask if the user wants to run the
% processing for BST only data (0) or Taq+BST data (1). Type in the choice
% and press "Enter".
% @parameter that can be changed:
%   directories of POI tables should be correctly given
%   out_dir
%   graph graphs if True
%   save_image Saves called mutation map

% Last Modified: 5/31/2019


%%  Setups
out_dir = 'D:\\MutationRateProject\\_20190529_%s_analysis';
...
if isTaq
    ...
    ratios_low = ones(3, 12) * 0.26;  % Lower threshold
    ratios_high = ones(3, 12) * 1.50; % Upper threshold
    to_gta_cutoff = 0.30;             % Mutation to GTA threshold
    to_c_cutoff = 0.4;                % Mutation to C threshold
    c_low = 0.50;
else % isBST
...
poi_counts = ones(120, 12) .* poi_counts;
...
% Go over FOV by FOV, process data and count mutations
for ifov = FOVs
    if save_image
        mkdir(fullfile(out_dir, sprintf('FOV%d', ifov)));
    end
    out_directory = fullfile(out_dir, sprintf('FOV%d', ifov));
    % Find POI's from the FOV processing in the iteration
    ...
    iMut = cell(4, 12);
    % Go through the FOV cycle by cycle and count mutations
    for icyc = cyc
        % Variable to keep track of the number of POI's not going to be
        % included in the total POI counts
        premut = zeros(poi_counts(ifov, icyc), 1);
        % Find which channel is the true channel intensity
        for ich = 1:4
        ...
        if chN == 4 % when C cycle
            ...
            for iich = channel
                if chN ~= iich
                    ...
                    % find pre_mutations
                    premut_idx = find(mut_itst > main_itst * ratios_high(iich, icyc));
                    premut(premut_idx) = 1;
                    % record the number of mutations
                    iMut{iich, icyc} = idx;
                    if save_image
        ...
        % Subtract the number of POI's that are not going to be included in
        % the total POI's
        poi_counts(ifov, icyc) = poi_counts(ifov, icyc) - sum(premut);
    end
    % Stroe mutation counts
...
if just_one
    final_rate = compile_mutation_rate(nMutations, FOVs, poi_counts);
    [cycle_mutation_rate, by_channel_mutation_rates] = compile_cycle_mutation_rate(nMutations, FOVs, poi_counts);
...
function final_rate = compile_mutation_rate(mutation_table, fov_list, poi_counts)
    % Counts total mutation counts
    count = zeros(4, 12);
    for iFov = fov_list
    ...
    for icyc = 1:4
        cyc = cycles{icyc};
        final_rate(icyc, :) = sum(count(:, cyc), 2) / sum(sum(poi_counts(:, cyc)));
        fprintf('%s : %d\n', bases(icyc), sum(sum(poi_counts(:, cyc))));
    end
...
function [cycle_mutation_rate, by_channel_mutation_rates] = compile_cycle_mutation_rate(mutation_table, fov_list, poi_counts)
    % Count total mutation counts
    by_channel_mutation_rates = zeros(4, 12, size(poi_counts, 1));
    for iFov = fov_list
    ...
    for icyc = 1:12
        cycle_mutation_rate(:, icyc) = sum(count(:, icyc), 2) / sum(poi_counts(:, icyc));
    end
end

function compile_results_with_error_bars_8(counts, fovs, file_dir, poi_counts)
    n_groups = floor(length(fovs) / 8);
    count = cell(n_groups, 1);
    nPoi_list = cell(n_groups, 1);
    % Split FOV's into groups of 8 and calculate mutation rates
    for igroup = 0:n_groups - 1
        ...
        nPoi_list{igroup+1} = sum(poi_counts(ifov, :), 1);
    end
...
function compile_results_with_error_bars(mutation_rates, Fovs, file_dir)
    ...
    % A
    mut_to = [];
    % Collect Mutation ratios
    for ig = Fovs
        mut_to = [mut_to, mutation_rates{ig}(:, cycles{3})];
    end
    % Acquire standard deviation and mean
